    ...
    <div className="p-10 max-w-5xl mx-auto h-full flex flex-col">
      <header className="mb-10 flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-400 mt-2">Firm diary and court schedules for future dates.</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-4">
        {loading ? (
          ...
          <div className="text-slate-600 border border-dashed border-white/10 rounded-lg p-10 text-center">No upcoming events scheduled.</div>
        ) : (
          upcoming.map(e => (
            <div key={e.id} className="bg-[#151619] border border-white/5 p-6 rounded-xl shadow-lg flex gap-6 hover:border-emerald-500/30 transition-colors group relative">
              <div className="flex flex-col items-center justify-center p-4 bg-black/30 rounded-lg min-w-24">
                <span className="text-xs uppercase text-slate-400 font-semibold mb-1">
                   {new Date(e.date).toLocaleString('default', { month: 'short' })}
                </span>
                <span className="text-3xl text-emerald-400 font-light leading-none">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] uppercase tracking-widest bg-emerald-500/10 text-emerald-400 px-2 py-1 rounded border border-emerald-500/20">{e.type}</span>
                ...
                <div className="flex gap-4 mb-3 text-sm text-slate-400">
                  ...
                  {e.case_title && <div className="flex items-center gap-1.5 text-emerald-500/80"><LinkIcon className="w-3 h-3" /> {e.case_title}</div>}
                </div>
                {e.description && <p className="text-slate-300 text-sm whitespace-pre-wrap">{e.description}</p>}
              </div>
            </div>
          ))
        )}
